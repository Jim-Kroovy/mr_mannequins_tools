...
def get_digitigrade_defaults():
    ...
    defaults = {
        ...
        'shapes' : [
            ...
            {'name' : 'target', 'mesh' : 'BALL_QUARTER', 'translation' : (0.0, 0.0, 0.0), 'rotation' : (0.0, 0.0, 0.0), 'scale' : (1.0, 1.0, 1.0), 'space' : 'CHAIN', 'forward' : '+Y', 'upward' : '+X'},
            {'name' : 'parent', 'mesh' : 'SQUING_HALF', 'translation' : (0.0, 0.0, 0.0), 'rotation' : (0.0, 0.0, 0.0), 'scale' : (1.5, 1.5, 1.5), 'space' : 'CHAIN', 'forward' : '+Y', 'upward' : '+X'},
            ...
            {'name' : 'forward', 'mesh' : 'RING_HALF', 'translation' : (0.0, 0.5, 0.0), 'rotation' : (0.0, 0.0, 0.0), 'scale' : (1.0, 1.0, 1.0), 'space' : 'CHAIN', 'forward' : '+Y', 'upward' : '+X'},
            ...
            {'name' : 'ending', 'mesh' : 'RING_QUARTER', 'translation' : (0.0, 0.0, 0.0), 'rotation' : (0.0, 0.0, 0.0), 'scale' : (1.0, 1.0, 1.0), 'space' : 'CHAIN', 'forward' : '+Y', 'upward' : '+X'},
            {'name' : 'floor', 'mesh' : 'CIRCLE_HALF', 'translation' : (0.0, 0.0, 0.0), 'rotation' : (0.0, 0.0, 0.0), 'scale' : (1.0, 1.0, 1.0), 'space' : 'CHAIN', 'forward' : '-Y', 'upward' : '+X'},
            {'name' : 'root', 'mesh' : 'CIRCLE_FULL', 'translation' : (0.0, 0.0, 0.0), 'rotation' : (0.0, 0.0, 0.0), 'scale' : (1.0, 1.0, 1.0), 'space' : 'WORLD', 'forward' : '+Z', 'upward' : '-Y'},
            ]
        }
    return defaults

def add_digitigrade_bones(oriented, rig, settings):
    """add digitigrade rigging bones (including the ik target parent)..."""
    ...
    # get the oriented bones the target bones are created from...
    first_eb = oriented.data.edit_bones.get(suffices['first'].affix)
    ending_eb = oriented.data.edit_bones.get(suffices['ending'].affix)

    # get the foot direction and distance... (flattened on Z axis and accounting for children)...
    start = Vector((ending_eb.head.x, ending_eb.head.y, 0.0))
    end = start
    if ending_eb.children:
        heads = [Vector((eb.head.x, eb.head.y, 0.0)) for eb in ending_eb.children]
        end = sum(heads, Vector((0.0, 0.0, 0.0))) / len(heads)
    ...
    # add the paw parent bone on the floor oriented along the inverse paw direction... (never parented, child of instead)...
    ...
    # add the paw forward bone pointing straight down...
    name = prefices['forward'].affix + suffices['ending'].affix
    head, tail = ending_eb.head, Vector((ending_eb.head.x, ending_eb.head.y, 0.0))
    rolls, parent = [direction, -direction], prefices['forward'].affix + suffices[chain[-1]].affix
    _functions.add_rigging_bone(rig, name, parent, head, tail, rolls, scale='ALIGNED')
    ...
    # and the inverse paw bone identically...
    ...
    # and the offset paw bone identically... (chains to its own kind, same as inverse above)...
    ...
    added[name] = {'groups' : ['tertiary'], 'color' : 'offset', 'shape' : 'offset'}
     ...
     # add the paw target bone identically...
    name = prefices['target'].affix + suffices['ending'].affix
    parent = prefices['parent'].affix + suffices['ending'].affix
    _functions.add_rigging_bone(rig, name, parent, head, tail, rolls, scale='ALIGNED')
    ...
    # create the floor bone on the floor the same as the paw parent bone...
    if settings.floor:
        name = prefices['floor'].affix + suffices['ending'].affix
        head = start + (direction * (distance * 0.5))
        tail = head + (direction * (distance * -1))
...
def set_digitigrade_bones(oriented, rig, settings):
    """set rigging constraints and drivers (edit mode, after updating new bones)..."""

    suffices, prefices = settings.suffices, settings.prefices
    chain = [s.name for s in suffices if s.name not in {'origin', 'ending', 'root'}]
    ...
    # the paw inverse bone copies the rotation of the target... (replaced parent space)...
    name = prefices['inverse'].affix + suffices['ending'].affix
    ...
    # the paw offset bone copies the location and rotation of it's inverse bone in world space...
    name = prefices['offset'].affix + suffices['ending'].affix
    ...
    # the paw forwards bone copies the rotation of it's offset bone... (before original)...
    name = prefices['forward'].affix + suffices['ending'].affix
    ...
    # and the paw forwards bone copies the offset bones location too... (off by default, local to local space)...
    ...
    # if we have an ik root bone the paw parent, pole and floor (if any) get a child of to it too...
    if suffices['root'].affix:
        root_bb = rig.data.bones.get(prefices['root'].affix + suffices['root'].affix)
        paw_parent = prefices['parent'].affix + suffices['ending'].affix
        names = [paw_parent, pole]
        if settings.floor:
        ...
        # the pole and floor (not the paw parent itself) get their root influence driven from the paw parents...
        for name in names[1:]:
            ...
            var.targets[0].id = rig
            var.targets[0].data_path = 'pose.bones["' + paw_parent + '"].constraints["Root Transform"].influence'
            if name == pole:
    ...
    # and the target parent has a floor constraint...
    if settings.floor:
...
def add_digitigrade_rigging(oriented, rig, settings):
    # define the unique rigging module identifier...
    chain = [s.name for s in settings.suffices if s.name not in {'origin', 'ending', 'root'}]
    module = "Digitigrade (" + settings.suffices[chain[0]].affix + " - " + settings.suffices['ending'].affix + ")"
    ...
    added = add_digitigrade_bones(oriented, rig, settings)
    rig.update_from_editmode()
    ...
    points = [rig.data.edit_bones[settings.suffices[n].affix].head for n in chain] + [ending_eb.head]
    ...
    set_digitigrade_bones(oriented, rig, settings)
    ...
    defaults = get_digitigrade_defaults()
    ...
    for name, items in added.items():
        ...
        if items:
            ...
            _functions.set_bone_shape(rig, name, settings.shapes[items['shape']], [tertiary, primary, secondary], 'DIGITIGRADE', settings.offset, defaults['offset'], reference)
...
def invoke_digitigrade_module(context, module):
    defaults = get_digitigrade_defaults()
    ...
    if len(selection) < 4:
        _functions.add_chain_names(module.settings, 3, ending=True)
        # digitigrade needs at least a thigh, calf, foot and paw bone to form a valid ik chain...
        print("Select at least four chain bones (e.g. thigh, calf, foot, paw).")
        return True
    ...
    # last selected bone is always the ending...
...
def draw_digitigrade_settings(layout, oriented, module):
    # define the digitigrade only settings to show...
...
def execute_digitigrade_module(context, module):
    ...
    # check the rigging for digitigrade specific conflicts... (checks constraints and collections)...
    ...
    name, added = add_digitigrade_rigging(oriented, rig, module.settings)
    # tagging module with its own real identifier first keeps these settings anchored under it...
    module.module = name
    if do_mirror:
        # check the mirrored rigging for digitigrade specific conflicts...
        ...
        if not all(validity.values()):
            ...
            if all(validity.values()):
                ...
                mirrored_name, mirrored_added = add_digitigrade_rigging(oriented, rig, module.mirrored)
...
def draw_digitigrade_controls(layout, rig, module):
    ...
    # the ending (paw) forward bone is general, not limited, and is parented to the last chain bone...
    ...
    # pole and paw parent bone names are built from the modules own fixed default affixes...
    defaults = get_digitigrade_defaults()
...
def set_snapped_direct(context, module):
    return _functions.set_snapped_chain_direct(context, module, parented=True, seed=True)
...
def set_snapped_kinematics(context, module, clear=True):
    ...
    forwards = [rig.pose.bones.get(prefices['forward'].affix + suffices[n].affix) for n in chain] + \
        [rig.pose.bones.get(prefices['forward'].affix + suffices['ending'].affix)]
    ...
    # unlike opposable, target here is parented to parent (not free-floating), so capture its live relative offset before touching anything and solve parent instead of target directly...
    target_relative = parent_pb.matrix.inverted() @ target_pb.matrix
    ...
    # solve parent so target, still sat at its preserved relative offset, ends up back on the paws current position via the same "Root Transform" solver target/pole/vector all use...
    desired_parent_matrix = ending_matrix @ target_relative.inverted()
    parent_local = _functions.get_compensated_local(parent_pb, desired_parent_matrix, lambda pb: pb.matrix.copy())
