...
def get_opposable_defaults():
    ...
    defaults = {
        ...
        'offset' : 0.3, 'position' : 0.5, 'mirror' : False, 'floor' : False, 'axis' : 'AUTO',
        # default prefices...
        'prefices' : [
        ...
        'shapes' : [
            {'name' : 'origin', 'mesh' : 'RING_HALF', 'translation' : (0.0, 0.75, 0.0), 'rotation' : (0.0, 0.0, 0.0), 'scale' : (2.0, 2.0, 2.0), 'space' : 'CHAIN', 'forward' : '+Y', 'upward' : '+X'},
            {'name' : 'target', 'mesh' : 'RING_HALF', 'translation' : (0.0, 0.0, 0.0), 'rotation' : (0.0, 0.0, 0.0), 'scale' : (2.0, 2.0, 2.0), 'space' : 'CHAIN', 'forward' : '+Y', 'upward' : '+X'},
            ...
            {'name' : 'forward', 'mesh' : 'RING_HALF', 'translation' : (0.0, 0.3, 0.0), 'rotation' : (0.0, 0.0, 0.0), 'scale' : (1.0, 1.0, 1.0), 'space' : 'CHAIN', 'forward' : '+Y', 'upward' : '+X'},
            ...
            {'name' : 'ending', 'mesh' : 'RING_HALF', 'translation' : (0.0, 0.0, 0.0), 'rotation' : (0.0, 0.0, 0.0), 'scale' : (1.5, 1.5, 1.5), 'space' : 'CHAIN', 'forward' : '+Y', 'upward' : '+X'},
            {'name' : 'floor', 'mesh' : 'CIRCLE_HALF', 'translation' : (0.0, 0.0, 0.0), 'rotation' : (0.0, 0.0, 0.0), 'scale' : (2.0, 2.0, 2.0), 'space' : 'CHAIN', 'forward' : '-Y', 'upward' : '+X'},
            {'name' : 'root', 'mesh' : 'CIRCLE_FULL', 'translation' : (0.0, 0.0, 0.0), 'rotation' : (0.0, 0.0, 0.0), 'scale' : (1.0, 1.0, 1.0), 'space' : 'WORLD', 'forward' : '+Z', 'upward' : '-Y'},
            ]
        }
    return defaults

def add_opposable_bones(oriented, rig, settings):
    """add opposable rigging bones..."""
    ...
    # get the first and last chain bone from the rigging armature and ending from the oriented...
    first_eb = rig.data.edit_bones[prefices['forward'].affix + suffices[chain[0]].affix]
    last_eb = rig.data.edit_bones[prefices['forward'].affix + suffices[chain[-1]].affix]
    ending_eb = oriented.data.edit_bones[suffices['ending'].affix]
    ...
    # add the ending forward bone oriented to the closest ending direction and shortest roll...
    name = prefices['forward'].affix + suffices['ending'].affix
    head, tail = ending_eb.head, _functions.get_bone_tail(ending_eb, None, recursive=True)
    axes = _functions.get_roll_axes(ending_eb, ending_eb.head, tail)
    parent = prefices['forward'].affix + suffices[chain[-1]].affix
    rolls = [utilities.functions.get_closest_vector(last_eb.z_axis, axes.values())]
    _functions.add_rigging_bone(rig, name, parent, head, tail, rolls, scale='ALIGNED')
    ...
    # and the inverse ending bone identically...
    ...
    # and the offset ending bone identically... (chains to its own kind, same as inverse above)...
    ...
    # and the ending ik target identically... (never parented to the root, that's done with a child of instead)...
    name = prefices['target'].affix + suffices['ending'].affix
    _functions.add_rigging_bone(rig, name, "", head, tail, rolls, scale='ALIGNED')
    ...
    # create the floor bone on the floor oriented along the secondary axis... (closest to armature Y)...
    if settings.floor:
        name = prefices['floor'].affix + suffices['ending'].affix
        head = Vector((ending_eb.head.x, ending_eb.head.y, 0.0))
        closest = utilities.functions.get_closest_vector(Vector((0.0, 1.0, 0.0)), [secondary, secondary * -1])
        tail = head + (closest * ending_eb.length)
...
def set_opposable_bones(oriented, rig, settings):
    ...
    # the hand inverse bone copies the rotation of the target... (replaced parent space)...
    name = prefices['inverse'].affix + suffices['ending'].affix
    ...
    # the hand offset bone copies the location and rotation of it's inverse bone in world space...
    name = prefices['offset'].affix + suffices['ending'].affix
    ...
    # the hand forwards bone copies the rotation of it's offset bone... (before original)...
    name = prefices['forward'].affix + suffices['ending'].affix
    ...
    # and the hand forwards bone copies the offset bones location too... (off by default, local to local space)...
    ...
    _functions.add_location_constraints(rig, name, copy, {})
    # the hand oriented bone inherits transforms from their rigging bone... (isolated to child of)...
    ...
    # if we have an ik root bone the target, pole and floor (if any) get a child of to it too...
    if suffices['root'].affix:
        ...
        # the pole and floor (not the target itself) get their root influence driven from the targets...
        for name in names[1:]:
            ...
            var.targets[0].id = rig
            var.targets[0].data_path = 'pose.bones["' + target + '"].constraints["Root Transform"].influence'
            if name == pole:
    ...
    # and the hand target has a floor constraint...
    if settings.floor:
...
def add_opposable_rigging(oriented, rig, settings):
    # define the unique rigging module identifier...
    chain = [s.name for s in settings.suffices if s.name not in {'origin', 'ending', 'root'}]
    module = "Opposable (" + settings.suffices[chain[0]].affix + " - " + settings.suffices['ending'].affix + ")"
    ...
    added = add_opposable_bones(oriented, rig, settings)
    rig.update_from_editmode()
    ...
    set_opposable_bones(oriented, rig, settings)
    ...
    defaults = get_opposable_defaults()
    ...
    for name, items in added.items():
        ...
        if items:
            ...
            _functions.set_bone_shape(rig, name, settings.shapes[items['shape']], [tertiary, primary, secondary], 'OPPOSABLE', settings.offset, defaults['offset'], reference)
...
def invoke_opposable_module(context, module):
    defaults = get_opposable_defaults()
    ...
    if len(selection) < 3:
        ...
        # opposable needs at least an upper, lower and hand bone to form a valid ik chain...
        print("Select at least three chain bones (e.g. upperarm, lowerarm, hand).")
        return True
    ...
    # last selected bone is always the chain ending...
...
def draw_opposable_settings(layout, oriented, module):
    # define the opposable only settings to show...
...
def execute_opposable_module(context, module):
    ...
    name, added = add_opposable_rigging(oriented, rig, module.settings)
    ...
    if do_mirror:
        ...
        if not all(validity.values()):
            ...
            if all(validity.values()):
                ...
                mirrored_name, mirrored_added = add_opposable_rigging(oriented, rig, module.mirrored)
...
def draw_opposable_controls(layout, rig, module):
    ...
    # the ending (hand) forward bone is general, not limited, and is parented to the last chain bone...
    ...
    # target, pole and floor bone names are built from the modules own fixed default affixes...
    defaults = get_opposable_defaults()
    ...
    if target_pb:
        root_transform = target_pb.constraints.get("Root Transform")
        if root_transform:
    ...
    for index, name in enumerate(ordered):
        ...
        header.label(text=label + " Bone") # + name
        if panel:
...
def set_snapped_direct(context, module):
    return _functions.set_snapped_chain_direct(context, module)
...
def set_snapped_kinematics(context, module, clear=True):
    ...
    _functions.set_snapped_target(target_pb, ending_matrix, lambda pb: pb.matrix.copy())
